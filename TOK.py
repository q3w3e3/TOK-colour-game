...
def on_resize(width, height):
    window.set_size(window.height, window.height)
    window.clear()
...
def on_key_press(symbol, modifiers):
    ...
    if symbol == key.SPACE:
        ...
        for a in xrange (0,360,30):
            circle((math.sin(math.radians(a))*(window.width//2.5))+(window.width//2),(math.cos(math.radians(a))*(window.height//2.5))+(window.height//2),window.height//12,g,b)
        r = random.randrange(0,360,30)
        print r,'degrees'
        circle((math.sin(math.radians(r))*(window.width//2.5))+(window.width//2),(math.cos(math.radians(r))*(window.height//2.5))+(window.height//2),window.height//12,(g+(1-(random.gauss(0,1)))),(b+(1-(random.gauss(0,1)))))
pyglet.app.run()
